...
function AnomalyAlertsPage() {
  ...
  const fetchAnomalyAlerts = useCallback(async () => {
    ...
    try {
      ...
      console.warn("Failed to load live anomalies, displaying connection error:", err.message);
      setError("Unable to load anomalies. Connection with AI microservice failed.");
    } finally {
      setIsLoading(false);
    }
  }, [demoMode]);
